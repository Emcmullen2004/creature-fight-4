 ...
 *                  - if used against an electric type... math expression)/
 ...
 *                      - Should do two times 75 points of damage (DON'T DO THE MATH! Write the
 *                      - Should do 0 damage
...
public class Golem extends Creature
{
    ...
    public void act() 
    {
        ...
        if( getHealthBar().getCurrent() <= 0 )
        {
            ...
            if( playerWorld.getNewOneCreature(1).getHealthBar().getCurrent() >= 0)
            {
                ...
                playerWorld.showText( "", getX()/2, getY()/2);
    ...
    public void attack(int idx)
    {
        ...
        else
        {
            ...
            if(enemyType.equalsIgnoreCase("Electric"))
            {
                 ...
                 */
                getWorld().showText("The attack was super effective! gg, no reee", getWorld().getWidth()/2, getWorld().getHeight()/2+26);
            ...
            else if(enemyType.equalsIgnoreCase("Flying"))
            {
                ...
                getWorld().showText("the attacked missed BOUI!", getWorld().getWidth()/2, getWorld().getHeight()/2+26);
    ...
    public void switchCreatures(int idx)
    {
        ...
        if(getHealthBar().current <= 0)
        {
            JOptionPane.showMessageDialog(null, "The Creature you have attempted to switch to dun got beaned. I would say use a revive but that ain't it chief. Chose a different creature");
        }
        else
        {
            while(getX() >= 0)
    ...
    {
        getImage().setTransparency(1000);
        getHealthBar().getImage().setTransparency(1000);
